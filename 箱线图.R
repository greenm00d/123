########简单出图#####################
####################################
# setwd("C:\\Users\\PC\\Desktop\\相关性\\pCR") # 设置路径，设置的路径是到桌面的，后边导出的东西在桌面上
# # 直接从CRAN中安装
...
rm(list = ls())
# 加载鸢尾花数据
iris <- read.csv('ALL_PCR_TEC.csv')
names(iris)
# 查看品种
table(iris$pCR)
# 箱线图叠加散点图展示不同品种花瓣长度的差异
ggplot(iris, # 用来画图的数据集名称
       aes(pCR, # 数据集的pCR列作为横坐标
...
# list = list(c("setpCRa","versicolor"),c("virginica","versicolor"),c("setpCRa","virginica")) 
list = list(c("no","yes")) 

# 把数据转成纵性数据
df=reshape2::melt(iris, # 需要转换的数据
                  id.vars=13, # 第5列pCR不动,这里是横坐标，用来分组的特征
                  measure.vars=15) # 第1列到第4列全部转换成纵性数据

ggplot(df, # 用来画图的数据集名称
       aes(pCR, # 数据集的pCR列作为横坐标
           value,  # 数据集的value列作为纵坐标
           color = pCR, # 根据数据集的pCR列设置箱线图的边框颜色
           fill = pCR))+ # 根据数据集的pCR列设置散点图和箱线图的填充颜色
  ...
  theme(strip.background = element_blank(), #把头上的灰框去掉
        axis.text = element_text(color="black", size=10, face = "bold"),# 加入 face = "bold" 使文字加粗
        axis.line = element_blank(),# 把横纵坐标的字体颜色改为黑色，看起来更沉稳
        axis.title = element_text(face = "bold")) 
ggsave("TEC-pCR.pdf", width = 5, height = 6) # 导出pdf
